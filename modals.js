...
<div class="modal-overlay" id="m-rehab" onclick="bgClose(event,'m-rehab')">
  <div class="modal">
    ...
    <div style="background:var(--abg);border:1px solid var(--aborder);border-radius:var(--r);padding:10px;margin-bottom:13px;font-size:11px;color:var(--amber)">Sẽ được cấp mã R-XXX tự động / 移入後自動分配編號</div>
...
<!-- 修整區移回 -->
<div class="modal-overlay" id="m-releaserhab" onclick="bgClose(event,'m-releaserhab')">
  <div class="modal">
    <div class="mhandle"></div>
    <div class="mtitle">Chuyển lại kho / 移回庫存</div>
    <input type="hidden" id="rr-rid">
    <div id="rr-info" style="margin-bottom:12px;padding:11px;background:var(--bg3);border-radius:var(--r);font-size:13px"></div>
    <div class="field"><label class="flabel">Chuyển về / 移回至</label>
      <select class="inp" id="rr-loc">
        <option value="mine">Nhà chủ / 我家</option>
        <option value="quang">Nhà Quang / Quang家</option>
        <option value="helper">Nhà trợ lý / 小幫手家</option>
      </select>
...
<div class="modal-overlay" id="m-newpur" onclick="bgClose(event,'m-newpur')">
  ...
  `;
  // Set today's date for purchase form
  const purDate = document.getElementById('pur-date');
  if (purDate) purDate.value = new Date().toISOString().split('T')[0];
